...
ATTR_FAN_READ_PERCENTAGE = "manual_speed"
...
class EcoventFan(FanEntity):
    ...
    def extra_state_attributes(self):
        ...
        return {
            ATTR_FAN_READ_PERCENTAGE: self._fan.man_speed,
    ...
    def set_percentage(self, percentage: int) -> None:
        if percentage is None:
            current_percentage = self._fan.man_speed
            current_percentage_digits = filter(str.isdigit, current_percentage)
            percentage = int("".join(current_percentage_digits))
        if percentage is None:
            percentage = 2
        if percentage < 1:
            percentage = 2
